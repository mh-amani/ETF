...
class GenIEFlanT5PL(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        if linearization_class_id is None:  # For backward compatibility
    ...
    def _get_default_collator(self):
        return RebelCollator(tokenizer=self.tokenizer, **self.hparams.default_collator_parameters)
    ...
    #             triplets = TripletUtils.convert_text_sequence_to_text_triplets(text)
    #
    ...
    #         sample["textual_triplets"] = TripletUtils.convert_text_sequence_to_text_triplets(sample["text"])
    #         if entity_name2id is not None and relation_name2id is not None:
    ...
    #     return output_obj
    #
    # def tokenize(self, src_text, tgt_text=None):
    ...
    #     model_kwargs = {}
    #
    #     model_inputs = {k: v.to(self.model.device) for k, v in self.tokenizer(src_text, **tokenizer_params).items()}
    #
    ...
    #         tokenizer_params["max_length"] = self.params["max_output_length"]
    #
    #         with self.tokenizer.as_target_tokenizer():
    #             decoder_inputs = {
    #                 k: v.to(self.model.device) for k, v in self.tokenizer(tgt_text, **tokenizer_params).items()
    #             }
    #
    #         model_kwargs["labels"] = decoder_inputs["input_ids"]
    #         model_kwargs["decoder_input_ids"] = self.model.prepare_decoder_input_ids_from_labels(model_kwargs["labels"])
        ...
        **kwargs,
        ...
        if input_is_processed_batch:
            ...
            if "raw" in input_data:
                input_text = [sample["text"] for sample in input_data["raw"]]
            else:
                input_text = None
        else:
        ...
        # ToDo: Make the following two lines more verbose and cleaner
        if prefix_allowed_tokens_fn is None and not force_free_generation and self.constraint_module is not None:
            prefix_allowed_tokens_fn = self.constraint_module.get_prefix_allowed_tokens_fn(batch=None)
        ...
        #     results["grouped_decoded_triplets"] = general_helpers.chunk_elements(triplets, num_return_sequences)
